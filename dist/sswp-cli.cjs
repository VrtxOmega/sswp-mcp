...
function sourceFiles(root) {
  ...
  if (tracked !== null) {
    ...
    for (const name2 of tracked) candidates.add(name2);
    for (const name2 of other) if (!name2.split("/").some((part) => excluded.has(part))) candidates.add(name2);
  } else {
    const walk = (folder) => {
      for (const name2 of (0, import_node_fs.readdirSync)(folder)) {
        if (excluded.has(name2)) continue;
        const path = (0, import_node_path.join)(folder, name2), stat = (0, import_node_fs.lstatSync)(path);
  ...
  const result2 = [];
  for (const name2 of [...candidates].sort((a, b) => Buffer.compare(Buffer.from(a), Buffer.from(b)))) {
    const parts = name2.split("/");
    if ((0, import_node_path.isAbsolute)(name2) || parts.includes("..")) throw Error("Source path escapes the approved root");
    ...
    if (stat.isFile()) result2.push(name2);
...
function sourceIdentity(root) {
  ...
  for (const name2 of sourceFiles(root)) hash.update(name2 + "\0" + (0, import_node_crypto.createHash)("sha256").update((0, import_node_fs.readFileSync)((0, import_node_path.join)(root, name2))).digest("hex") + "\n");
  return hash.digest("hex");
}
function copySources(root, dest) {
  for (const name2 of sourceFiles(root)) {
    const target = (0, import_node_path.join)(dest, name2);
    (0, import_node_fs.mkdirSync)((0, import_node_path.dirname)(target), { recursive: true });
    (0, import_node_fs.copyFileSync)((0, import_node_path.join)(root, name2), target);
  }
}
...
function outputManifest(root, outputs) {
  ...
  const walk = (p) => {
    ...
    if (stat.isDirectory()) {
      for (const name2 of (0, import_node_fs4.readdirSync)(p).sort()) walk((0, import_node_path4.join)(p, name2));
...
async function dispatch(name2, args) {
  const validate = validators.get(name2);
  if (!validate) throw Error("Unknown tool");
  if (!validate(args)) throw Error(ajv.errorsText(validate.errors));
  if (name2 === "sswp_witness") return witnessAction(args);
  if (name2 === "sswp_bulk_witness") {
    const result2 = [];
  ...
  if (name2 === "sswp_check_repo") return { git: await gitIntegrityGate(args.repoPath), lockfile: lockfileGate(args.repoPath) };
  if (name2 === "sswp_registry_health") return { nodes: REG.getHealthBoard().slice(0, args.limit || 20), integrity: REG.verifyLedger(), delivery_pending: REG.db.prepare("SELECT COUNT(*) n FROM event_outbox WHERE delivered=0").get(), materialization_pending: REG.db.prepare("SELECT COUNT(*) n FROM attestations WHERE json_extract(metadata,'$.materialization')='pending'").get() };
  if (name2 === "sswp_ledger") return { integrity: REG.verifyLedger(), entries: REG.getLedger(args.limit || 20) };
  if (name2 === "sswp_node_search") {
    const q = args.query.toLowerCase();
    return REG.listNodes().filter((n) => (n.name + " " + n.repo_path + " " + n.description).toLowerCase().includes(q)).slice(0, args.limit || 20);
  }
  if (name2 === "sswp_analyze_deps") return { assessment: "metadata signals only; not a security verdict", packages: args.packages.map((p) => ({ ...p, unpinned: /[\^~*<>]|latest/.test(p.version), integrityDeclaration: p.integrity ? "PRESENT_UNVERIFIED" : "MISSING", vulnerabilityStatus: "NOT_CHECKED" })) };
  if (name2 === "sswp_export_to_omega") {
    materializePending();
    REG.flushEvents();
...
// src/cli.ts
var [name, input2 = "{}"] = process.argv.slice(2);
dispatch(name, JSON.parse(input2)).then((r) => console.log(JSON.stringify(r, null, 2))).catch((e) => {
